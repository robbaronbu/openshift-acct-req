...
def update_openshift_rolebindings(
    token, api_url, project_name, role, rolebindings_json
):
    headers = {
        "Authorization": "Bearer " + token,
        "Accept": "application/json",
...
def update_user_role_project(token, api_url, project_name, user, role, op):
    ...
    if r.status_code == 200 or r.status_code == 201:
        ...
        r = update_openshift_rolebindings(
            token, api_url, project_name, openshift_role, role_binding
        )
